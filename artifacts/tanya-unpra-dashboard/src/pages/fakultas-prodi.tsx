...
import { Loader2, Plus, Edit, Trash2, School, BookOpen } from "lucide-react";
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <Card>
          <CardHeader className="flex flex-row items-start justify-between">
            <div className="space-y-1">
              ...
              <CardDescription>Daftar fakultas yang tersedia</CardDescription>
            </div>
          ...
          <CardContent className="p-0">
            {fLoading ? (
              ...
              <div className="divide-y">
                {fakultasList.map((f) => {
                  const prodiCount = prodiList.filter(p => p.fakultasId === f.id).length;
                  return (
                    <div key={f.id} className="flex items-center justify-between px-4 py-3">
                      <div>
                        ...
                        <div className="flex items-center gap-2 mt-1">
                          ...
                          <span className="text-xs text-muted-foreground">{prodiCount} prodi</span>
                        </div>
                      </div>
        ...
        <Card>
          <CardHeader className="flex flex-row items-start justify-between">
            <div className="space-y-1">
              <CardTitle className="flex items-center gap-2">
                <BookOpen className="h-5 w-5" /> Program Studi
              </CardTitle>
              <CardDescription>Daftar prodi yang tersedia</CardDescription>
            </div>
          ...
          <CardContent className="p-0">
            {pLoading ? (
              ...
              <p className="text-center text-muted-foreground py-8 text-sm">
                {fakultasList.length === 0 ? "Tambahkan fakultas terlebih dahulu." : "Belum ada prodi. Tambahkan dulu."}
              </p>
            ) : (
              <div className="divide-y">
                {prodiList.map((p) => (
                  <div key={p.id} className="flex items-center justify-between px-4 py-3">
                    <div>
                      ...
                      <div className="flex items-center gap-2 mt-1">
                        <Badge variant="secondary" className="text-xs">{p.singkatan}</Badge>
                        <span className="text-xs text-muted-foreground">{p.fakultasName ?? '-'}</span>
                      </div>
                    </div>
                    <div className="flex gap-1">
                      <Button variant="ghost" size="icon" className="h-8 w-8" onClick={() => {
